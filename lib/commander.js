...
Command.prototype.missingArgument = function(name){
  this.error();
  this.error("  error: missing required argument `%s'", name);
  this.error();
  this.exit(1)
};
...
Command.prototype.optionMissingArgument = function(option, flag){
  this.error();
  if (flag) {
    this.error("  error: option `%s' argument missing, got `%s'", option.flags, flag);
  } else {
    this.error("  error: option `%s' argument missing", option.flags);
  }
  this.error();
  this.exit(1);
};
...
Command.prototype.unknownOption = function(flag){
  this.error();
  this.error("  error: unknown option `%s'", flag);
  this.error();
  this.exit(1);
};
...
Command.prototype.error = console.err
